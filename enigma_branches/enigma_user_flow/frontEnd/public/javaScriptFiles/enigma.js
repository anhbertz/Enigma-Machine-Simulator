...
//TDOD: standarldzed var versus let
...
function handleRingSettings() {
  ...
  leftRingSettingIncrement.on("click", () => {
    handleSpecificRingAdjuster(leftRingSettingIncrement, $("#leftRingSettingValue"));
  });

  leftRingSettingDecrement.on("click", () => {
    handleSpecificRingAdjuster(leftRingSettingDecrement, $("#leftRingSettingValue"));
  });
  
  middleRingSettingIncrement.on("click", () => {
    handleSpecificRingAdjuster(middleRingSettingIncrement, $("#middleRingSettingValue"));
  });

  middleRingSettingDecrement.on("click", () => {
    handleSpecificRingAdjuster(middleRingSettingDecrement, $("#middleRingSettingValue"));
  });

  rightRingSettingIncrement.on("click", () => {
    handleSpecificRingAdjuster(rightRingSettingIncrement, $("#rightRingSettingValue"));
  });

  rightRingSettingDecrement.on("click", () => {
    handleSpecificRingAdjuster(rightRingSettingDecrement, $("#rightRingSettingValue"));
  });
}
 ...
 *
 * @todo Finish for the rest of the config settings
 */
function validateConfig() {
  try {
    // Validate rotor order/positions
    ROTOR_POSITIONS_NUMBERS.forEach((rotorNum, position) => {
      if (!(rotorNum >= MIN_ROTOR_NUMBER && rotorNum <= MAX_ROTOR_NUMBER))
        throw "Rotor selection error; make sure to select a rotor for each position in the machine.";
    });
  } catch (err) {
    alert(err);
    console.log(err);
...
async function keydown(e) {
  ...
  // Execute rotor rotatation. Currently does not account for other enigma settings on the backend
  await rotate();
}
...
function submitLogin() {
  ...
  // Load user's saved messages & configs into the right sidebar

  // Add an onclick event listener to all saved messages (for loading the configuration when clicked)
 ...
 *
 * @todo Integrate with actual backend machine state. Currently just doing a placeholder rotation.
 */
async function rotate() {
  ...
  let rotate3 = r2val == DIAL_2; // TODO: Double-check the conditions and the ì lôp in line 269
...
function plusRotor(button) {
  ...
  $(targetRotor).find("tr").each(function() {
    console.log($(this));
    $(this).find("td")[0].innerHTML = rotorInc(parseInt($(this).find("td")[0].innerHTML));
  });
}
...
function minusRotor(button) {
  ...
  $(targetRotor).find("tr").each(function() {
    $(this).find("td")[0].innerHTML = rotorDec(parseInt($(this).find("td")[0].innerHTML));
  });
}
