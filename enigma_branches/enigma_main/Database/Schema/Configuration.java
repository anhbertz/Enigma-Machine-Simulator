 ...
 *
 */
public class Configuration extends DatabaseConnector {
    private static final String dbTableName = "Configuration";
    private static final String primaryKeyColumnName = "configuration_id";
    private static final int primaryKeyColumnIdx = 1;

    // these member fields map directly to the Configuration DB table columns
    // with the same names, but prefixed with "configuration_" -- e.g., configuration_id.
    private int id;
    private String name;
     ...
     *   Configuration internal data structures.
     * Does NOT set the primary key
     * @param resSet   ResultSet of a Select * from the Configuration table.
...
//   and then putting it down with the appropriate public Setter.
        ...
        try {
            this.name = resSet.getString("configuration_name");
            success = true;
    ...
    /**
     * Inserts a single record into the Configuration db table, populating it with the values
...
// return result
        ...
        try {
            ...
            if (resultSet.next()) {
                ...
                this.name = resultSet.getString("configuration_name");
            } else {
                result = false;
            }
    ...
    /**
     * Updates a single record of the Configuration db table, populating it with the values
     *   stored in the current object instance.
     * Precondition: the dbConnectionMaker must have been set
...
// return result
        ...
        String updateSQL = "UPDATE " + dbTableName
                + " SET configuration_name = " + this.name
    ...
    /**
     * Deletes a single record from the Configuration db table, corresponding to the
...
// return result
    ...
    /**
     * Either inserts or updates a single record of the Configuration db table, depending on
...
// return the result
    ...
    /**
     * Reads a single record of the Configuration db table
     * Precondition: the dbConnectionMaker must have been set
...
// return result
    ...
    /**
     * Deletes the record of the Configuration db table corresponding to the current object instance
     * Precondition: the dbConnectionMaker must have been set
...
// return result
    ...
    /**
     * Setter for configuration_name table field
     * @param name_new
     * @return void
     */
    ...
    /**
     * Getter for configuration_name table field
     * @return String
     */
    ...
    /**
     * Constructor creates an empty Configuration object
     * @return  a valid Configuration object that does not map to a record in the DB
     */
    ...
    /**
     * Constructor creates a Configuration object, and attempts to load a record from the DB
     * @return IFF the queryPrimaryKey is valid, a Configuration object populated with the
     *         corresponding values for that record.
     *         otherwise, returns a valid but empty Configuration object
     */
    public Configuration (int queryPrimaryKey) {
        super();
